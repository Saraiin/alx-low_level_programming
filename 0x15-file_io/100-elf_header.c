...
/**
 * abiverself - print ELF ABI version
 * @buffer: the ELF header
 */
void abiverself(const unsigned char *buffer)
{
	printf("  %-34s %u\n", "ABI Version:", buffer[EI_ABIVERSION]);
}
/**
 * typeelf - print ELF type
 * @buffer: the ELF header
 * @big_endian: endianness (big endian if non-zero)
 */
void typeelf(const unsigned char *buffer, int big_endian)
{
	char *type_table[5] = {
		"NONE (No file type)",
...
/**
 * entryelf - print entry point address
 * @buffer: string containing the entry point address
 * @bit_mode: bit mode (32 or 64)
 * @big_endian: endianness (big endian if non-zero)
 */
void entryelf(const unsigned char *buffer, size_t bit_mode, int big_endian)
{
...
int main(int argc, const char *argv[])
{
	...
	readf(fd, (char *) buffer, 18);
	magicelf(buffer);
	bit_mode = clasself(buffer);
	big_endian = dataelf(buffer);
	versionelf(buffer);
	osabielf(buffer);
	abiverself(buffer);
	typeelf(buffer, big_endian);

	lseek(fd, 24, SEEK_SET);
	readf(fd, (char *) buffer, bit_mode / 8);
	entryelf(buffer, bit_mode, big_endian);
